        ...
        <tr v-for="category in categories" :key="category.id">
          ...
          <td class="d-flex justify-content-between">
            ...
            <button
              ...
              class="btn btn-link mr-2"
              @click.stop.prevent="
                updateCategory({ categoryId: category.id, categoryName: category.name })
              "
            >
              Save
            </button>
...
export default {
  ...
  methods: {
    ...
    async updateCategory({ categoryId, categoryName }) {
      try {
        const { data } = await adminAPI.categories.put({ categoryId, categoryName });
        console.log(data)
